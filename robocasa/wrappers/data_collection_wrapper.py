...
class DataCollectionWrapper(Wrapper):
    ...
    def set_policy_mode(self, policy_mode):
        assert policy_mode in [
            0,
            1,
        ], "Policy mode must be 0 (teleoperation) or 1 (exploration policy)"
    ...
    def _flush(self):
        ...
        non_robot_qpos_idx = set(
            range(self.env.sim.get_state().qpos.flatten().shape[0])
        ) - set(robot_indices)
    ...
    def reset(self):
        ...
        for _list in joint_list:
            for joint in _list:
                self.initial_obj_qpos[joint] = self.env.sim.data.get_joint_qpos(
                    joint
                ).copy()
