...
class HDF5VideoDataset(Dataset):
    ...
    def __init__(self, hdf5_path: str, camera_names: List[str], chunk_size: int = 100):
        ...
        print(f"Dataset initialized: {self.n_frames} frames, {self.frame_height}x{self.frame_width}")
    ...
    def _get_frame(self, frame_idx: int) -> np.ndarray:
        ...
        if (not hasattr(self._thread_local, 'chunk_data') or 
            self._thread_local.chunk_start != chunk_idx * self.chunk_size):
...
def generate_video_from_hdf5_with_dataloader(
    ...
    video_writer = cv2.VideoWriter(
        str(output_file), fourcc, 30.0, 
        (dataset.frame_width, dataset.frame_height)
    )
    
    print(f"Processing {len(dataset)} frames with batch_size={batch_size}, workers={num_workers}")
    ...
    if use_dataloader:
        ...
        with h5py.File(hdf5_path, "r") as output_hdf5:
            ...
            first_frame = np.concatenate(
                [first_data["obs/{}".format(camera_name)][0] for camera_name in camera_names], 
                axis=1
            )
            height, width = first_frame.shape[:2]
            print(f"Dimensions: {height}x{width}")
            
            video_writer = cv2.VideoWriter(str(output_file), fourcc, 30.0, (width, height))
            ...
            for data_key in data_keys:
                ...
                for camera_name in camera_names:
                    camera_data[camera_name] = data[f"obs/{camera_name}"][:]  # Load entire dataset
...
if __name__ == "__main__":
    ...
    hdf5_paths = [
        "/mnt/team-storage/rshah/datasets/robocasa/datasets/memory/MemFruitInSinkLeftFar/2025-07-13-15-44-32/demo_im1024_notp_highres.hdf5",
        "/mnt/team-storage/rshah/datasets/robocasa/datasets/memory/MemPutKBreadInMicrowave/2025-07-25-10-40-55/demo_im1024_notp_highres.hdf5",
        "/mnt/team-storage/rshah/datasets/robocasa/datasets/memory/MemFruitPickRightFar/2025-07-15-15-07-57/demo_im1024_notp_highres.hdf5",
        "/mnt/team-storage/rshah/datasets/robocasa/datasets/memory/MemWashAndReturnLeft/2025-07-25-00-12-14/demo_im1024_notp_highres.hdf5",
        "/mnt/team-storage/rshah/datasets/robocasa/datasets/memory/MemHeatPot/2025-07-24-22-26-20/demo_im1024_notp_highres.hdf5",
    ]
    
    output_directory = "/mnt/team-storage/rshah/datasets/robocasa/videos/memory"
