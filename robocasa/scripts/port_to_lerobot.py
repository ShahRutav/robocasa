...
def get_max_dims(
    hdf5_paths: list[str], keys_with_variable_length: list[str]
) -> dict[str, int]:
    max_dims = {}
        ...
        with h5py.File(hdf5_path, "r") as f:
            ...
            for demo_key in demo_keys:
                ...
                for key in keys_with_variable_length:
                    ...
                    max_dims[key] = (max(max_dims[key][0], data.shape[0]),) + tuple(
                        data.shape[1:]
                    )
    return max_dims
...
def discover_dataset_properties(hdf5_paths: list[str]) -> dict[str, Any]:
    ...
    keys_with_variable_length = [
        "states",
        "obs/object-state",
        "obs/objects-joint-state",
        "obs/object",
    ]
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        if "data" in f and "env_args" in f["data"].attrs:
            ...
            if env_args:
                ...
                fps = env_args.get(
                    "control_freq", 20
                )  # default to 20 fps as in robosuite if not found
                fps = int(fps)
...
def load_robocasa_episode(
    f: h5py.File, ep_name: str, other_keys: dict[str, tuple] = None
) -> dict[str, Any]:
    """
    ...
    for attr_name in ep_group.attrs:
        ...
        attr_value = (
            attr_value.decode("utf-8") if isinstance(attr_value, bytes) else attr_value
        )
...
def convert_robocasa_to_lerobot(
    ...
    if properties.get("other_keys"):
        print(
            f"  Other episode-level datasets: {list(properties['other_keys'].keys())}"
        )
    ...
    for key, value in features.items():
        print(
            f"  {key}: dtype={value.get('dtype', 'N/A')}, shape={value.get('shape', 'N/A')}"
        )
    ...
    for dataset_name, paths in dataset_groups.items():
        ...
        def custom_encode_video(
            video_key: str, episode_index: int, dataset_ref=dataset
        ):
            """Custom video encoding with specified codec backend."""
            temp_path = (
                Path(tempfile.mkdtemp(dir=dataset_ref.root))
        ...
        for dataset_path in paths:
            ...
            with h5py.File(dataset_path, "r") as f:
                ...
                for ep_name in demos:
                    ...
                    task_clip_embedding = get_clip_embedding(
                        lang, tokenizer, model, device
                    )
                    ...
                    for t in range(num_steps):
                        ...
                        for img_name in properties["image_names"]:
                            frame_data[OBS_IMAGES + "." + img_name] = ep_data["obs"][
                                img_name + "_image"
                            ][t]
                        ...
                        for state_key in properties["state_keys"]:
                            if "obs/" + state_key in properties["max_dims"]:
                                ...
                                frame_data[state_key][
                                    : ep_data["obs"][state_key][t].shape[0]
                                ] = ep_data["obs"][state_key][t]
                            else:
                        ...
                        for key in properties["other_keys"]:
                            ...
                            if key in properties["max_dims"]:
                                # pad it with -np.inf
                                frame_data[key] = np.full(
                                    properties["max_dims"][key], -np.inf
                                )
                        ...
                        frame_data[OBS_STATE] = np.concatenate(
                            [arm_state, gripper_state], axis=0
                        )
                        ...
                        lang = ep_data[
                            "lang"
                        ]  # only language instruction is available in general
                        frame_data["task"] = lang
                        frame_data["task_clip_embedding"] = torch.from_numpy(
                            task_clip_embedding
                        ).float()
...
def main(
    ...
    *,
    ...
    """
    dataset_paths = (
        [dataset_path] if isinstance(dataset_path, str) else list(dataset_path)
    )
